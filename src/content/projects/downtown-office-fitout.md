---
title: Downtown Office Fit-Out
slug: downtown-office-fitout
summary: A two-floor commercial fit-out delivered around an occupied building.
category: Construction
location: Your City, ST
year: 2024
cover: /images/projects/office-cover.jpg
gallery:
  - /images/projects/office-1.jpg
  - /images/projects/office-2.jpg
featured: true
seo:
  description: Downtown Office Fit-Out — two-floor commercial construction project by SKR Construction.
---

## Project overview

A growing firm needed two floors fitted out while the rest of the building stayed
occupied. We phased the work to minimize disruption and kept the common areas open
throughout.

### Scope

- Partition and structural alterations
- New HVAC and electrical
- Custom joinery and finishes
- Phased delivery around live operations

The fit-out was handed over ready for move-in, with zero days of business interruption.
